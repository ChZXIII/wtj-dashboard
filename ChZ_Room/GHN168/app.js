...
function createNewDocument() {
  if (!confirm('แกแน่ใจใช่ไหมว่าต้องการสร้างเอกสารใหม่? ข้อมูลฟิลด์ต่างๆ ที่กำลังพิมพ์อยู่จะถูกล้าง (แต่ข้อมูลที่เคยกดเซฟไว้แล้วจะไม่หายนะแก)')) {
    return;
  }
...
function fetchDocumentsFromSheets(showToast = false) {
  ...
  .then(([qtRes, ivRes]) => {
    ...
    if (mergedAny) {
      ...
      if (showToast) alert('ซิงค์ข้อมูลใบเสนอราคาและใบวางบิลจาก Google Sheets สำเร็จแล้วแก!');
    }
  })
  .catch(err => {
