...
let docItems = [];
let currentDocType = 'invoice'; // invoice, receipt (quotation removed)
...
function initDocumentGenerator() {
  ...
  if (docNumInput) {
    docNumInput.value = autoGenerateDocNumber('invoice');
  }
...
function autoGenerateDocNumber(type) {
  ...
  const prefix = type === 'invoice' ? 'IV' : 'RE';
...
function setDocType(type) {
  ...
  if (prevDepositTerms) {
    prevDepositTerms.style.display = 'none';
  }
...
function updateAutofillSelect() {
  ...
  if (currentDocType === 'receipt') {
    ...
    autofillGroup.style.display = (invoices.length > 0) ? 'flex' : 'none';
  } else {
