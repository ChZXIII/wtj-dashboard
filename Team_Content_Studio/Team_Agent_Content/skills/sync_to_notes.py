...
def compile_and_sync(topic_name):
    ...
    for d in possible_dirs:
        s_path = os.path.join(d, "5_youtube_script.md")
        r_path = os.path.join(d, "3_research_doc.md")
        st_path = os.path.join(d, "7_storyboard_and_prompts.md")
        if os.path.exists(s_path) and os.path.exists(r_path) and os.path.exists(st_path):
            workspace_dir = d
            script_path = s_path
            research_path = r_path
            storyboard_path = st_path
            break
