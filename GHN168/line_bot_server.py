...
def build_tax_filing_flex_message(tax_data: Dict[str, Any], tax_type: str = "all") -> Dict[str, Any]:
    """Constructs a modern, elegant LINE Flex Message displaying RD e-Filing Tax Report (ภ.พ.30, ภ.ง.ด.3, ภ.ง.ด.53)."""
    period = tax_data.get("period_label", "ปัจจุบัน")
    pnd30 = tax_data.get("pnd30", {})
    pnd3 = tax_data.get("pnd3", {})
    pnd53 = tax_data.get("pnd53", {})
    summary = tax_data.get("summary", {})

    sales_pre_vat = pnd30.get("sales_pre_vat", 0.0)
    vat_output = pnd30.get("vat_output", 0.0)
    purchases_pre_vat = pnd30.get("purchases_pre_vat", 0.0)
    vat_input = pnd30.get("vat_input", 0.0)
    vat_net_payable = pnd30.get("vat_net_payable", 0.0)

    pnd3_count = pnd3.get("count", 0)
    pnd3_base_total = pnd3.get("base_total", 0.0)
    pnd3_wht_total = pnd3.get("wht_total", 0.0)

    pnd53_count = pnd53.get("count", 0)
    pnd53_base_total = pnd53.get("base_total", 0.0)
    pnd53_wht_total = pnd53.get("wht_total", 0.0)

    total_tax_to_pay = summary.get("total_tax_to_pay", max(0.0, vat_net_payable) + pnd3_wht_total + pnd53_wht_total)

    vat_badge_color = "#dc2626" if vat_net_payable > 0 else ("#16a34a" if vat_net_payable < 0 else "#64748b")
    vat_badge_text = f"ชำระสุทธิ {vat_net_payable:,.2f} ฿" if vat_net_payable > 0 else (f"ภาษีซื้อยกไป {abs(vat_net_payable):,.2f} ฿" if vat_net_payable < 0 else "0.00 ฿")

    # Construct sections based on tax_type or all
    body_contents = []

    # Section 1: ภ.พ.30 (VAT 7%)
    if tax_type in ["all", "pnd30", "vat", "ภพ30", "ภ.พ.30"]:
        body_contents.append({
            "type": "box",
            "layout": "vertical",
            "paddingAll": "12px",
            ...
            "contents": [
                {
                    ...
                    "layout": "horizontal",
                    "contents": [
                        {"type": "text", "text": "1. 🏛️ ภ.พ.30 (ภาษีมูลค่าเพิ่ม 7%)", "size": "xs", "weight": "bold", "color": "#0f172a", "flex": 7},
                        {"type": "text", "text": vat_badge_text, "size": "xxs", "weight": "bold", "color": vat_badge_color, "align": "end", "flex": 5}
                    ]
                },
                {"type": "separator", "margin": "sm"},
                {
                    ...
                    "margin": "sm",
                    "contents": [
                        {"type": "text", "text": "• ยอดขายก่อน VAT:", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"{sales_pre_vat:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#1e293b", "align": "end", "flex": 4}
                    ]
                },
                {
                    "type": "box",
                    ...
                    "margin": "xs",
                    "contents": [
                        {"type": "text", "text": "• ภาษีขาย 7% (Output):", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"+{vat_output:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#dc2626", "align": "end", "flex": 4}
                    ]
                },
                {
                    "type": "box",
                    ...
                    "margin": "xs",
                    "contents": [
                        {"type": "text", "text": "• ยอดซื้อมี VAT:", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"{purchases_pre_vat:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#1e293b", "align": "end", "flex": 4}
                    ]
                },
                {
                    "type": "box",
                    ...
                    "margin": "xs",
                    "contents": [
                        {"type": "text", "text": "• ภาษีซื้อ 7% (Input):", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"-{vat_input:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#16a34a", "align": "end", "flex": 4}
                    ]
                }
            ]
        })

    # Section 2: ภ.ง.ด.3 (บุคคลธรรมดา)
    if tax_type in ["all", "pnd3", "ภงด3", "ภ.ง.ด.3", "wht3"]:
        body_contents.append({
            "type": "box",
            "layout": "vertical",
            ...
            "contents": [
                {
                    ...
                    "layout": "horizontal",
                    "contents": [
                        {"type": "text", "text": "2. 👤 ภ.ง.ด.3 (หัก ณ ที่จ่ายบุคคล)", "size": "xs", "weight": "bold", "color": "#1e40af", "flex": 7},
                        {"type": "text", "text": f"{pnd3_count} ราย", "size": "xxs", "color": "#3b82f6", "align": "end", "flex": 3}
                    ]
                },
                {"type": "separator", "margin": "sm"},
                {
                    ...
                    "margin": "sm",
                    "contents": [
                        {"type": "text", "text": "• ฐานจ่ายรวม (ฟรีแลนซ์/ทีมงาน):", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"{pnd3_base_total:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#1e293b", "align": "end", "flex": 4}
                    ]
                },
                {
                    "type": "box",
                    ...
                    "margin": "xs",
                    "contents": [
                        {"type": "text", "text": "• ภาษีหักนำส่ง (3%):", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"{pnd3_wht_total:,.2f} ฿", "size": "xs", "weight": "bold", "color": "#2563eb", "align": "end", "flex": 4}
                    ]
                }
            ]
        })

    # Section 3: ภ.ง.ด.53 (นิติบุคคล)
    if tax_type in ["all", "pnd53", "ภงด53", "ภ.ง.ด.53", "wht53"]:
        body_contents.append({
            "type": "box",
            "layout": "vertical",
            "margin": "sm" if body_contents else "none",
            ...
            "contents": [
                {
                    ...
                    "layout": "horizontal",
                    "contents": [
                        {"type": "text", "text": "3. 🏢 ภ.ง.ด.53 (หัก ณ ที่จ่ายนิติบุคคล)", "size": "xs", "weight": "bold", "color": "#6b21a8", "flex": 7},
                        {"type": "text", "text": f"{pnd53_count} ราย", "size": "xxs", "color": "#a855f7", "align": "end", "flex": 3}
                    ]
                },
                {"type": "separator", "margin": "sm"},
                {
                    ...
                    "margin": "sm",
                    "contents": [
                        {"type": "text", "text": "• ฐานจ่ายรวม (ค่าเช่า/บริการ บจก.):", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"{pnd53_base_total:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#1e293b", "align": "end", "flex": 4}
                    ]
                },
                {
                    "type": "box",
                    ...
                    "margin": "xs",
                    "contents": [
                        {"type": "text", "text": "• ภาษีหักนำส่ง (5%/3%/1%):", "size": "xxs", "color": "#64748b", "flex": 6},
                        {"type": "text", "text": f"{pnd53_wht_total:,.2f} ฿", "size": "xs", "weight": "bold", "color": "#7e22ce", "align": "end", "flex": 4}
                    ]
                }
            ]
        })
        ...
        "contents": [
            {
                ...
                "layout": "horizontal",
                "contents": [
                    {"type": "text", "text": "💰 รวมภาษีต้องนำส่งสรรพากร:", "size": "xs", "weight": "bold", "color": "#92400e", "flex": 6},
                    {"type": "text", "text": f"{total_tax_to_pay:,.2f} ฿", "size": "sm", "weight": "bold", "color": "#b45309", "align": "end", "flex": 4}
                ]
            },
            {
                "type": "text",
                "text": f"⏳ เดดไลน์ e-Filing: WHT 15/{tax_data.get('month', 1)+1 if tax_data.get('month', 1) < 12 else 1:02d} | VAT 23/{tax_data.get('month', 1)+1 if tax_data.get('month', 1) < 12 else 1:02d}",
                "size": "xxs",
                "color": "#a16207",
                "margin": "xs"
            }
    ...
    flex_bubble = {
        ...
        "header": {
            ...
            "paddingAll": "16px",
            "contents": [
                {"type": "text", "text": "GHN 168 TAX FILING SUITE", "color": "#94a3b8", "size": "xxs", "weight": "bold"},
                {"type": "text", "text": f"🏛️ สรุปตัวเลขยื่นภาษี e-Filing ({period})", "color": "#ffffff", "size": "md", "weight": "bold", "margin": "xs"},
                {"type": "text", "text": "บจ. จีเอชเอ็น 168 มีเดีย แอนด์ ครีเอชั่น (0505568016475)", "color": "#cbd5e1", "size": "xxs", "margin": "xs"}
            ]
        },
        "body": {
            "type": "box",
        ...
        "footer": {
            ...
            "contents": [
                {
                    "type": "text",
                    "text": "✅ ข้อมูลพร้อมคัดลอกยื่น RD e-Filing (ภ.พ.30, ภ.ง.ด.3, ภ.ง.ด.53) ได้ทันที",
                    "size": "xxs",
                    "color": "#16a34a",
                    "align": "center"
                }
    ...
    return {
        "type": "flex",
        "altText": f"🏛️ สรุปตัวเลขยื่นภาษี GHN168 ({period}): ภาษีนำส่งรวม {total_tax_to_pay:,.2f} ฿",
        "contents": flex_bubble
    }


def build_cpa_audit_pack_flex_message(audit_data: Dict[str, Any]) -> Dict[str, Any]:
    """Constructs a high-class LINE Flex Message for CPA Annual Audit Package (ปิดงบการเงินประจำปี)."""
    target_year = audit_data.get("year", datetime.now().year)
    pnl = audit_data.get("pnl_summary", {})
    doc_counts = audit_data.get("document_counts", {})
    readiness = audit_data.get("audit_readiness", {})
    drive_folders = audit_data.get("drive_folders", {})

    rev_pre = pnl.get("total_revenue_pre_vat", 0.0)
    exp_pre = pnl.get("total_expense_pre_vat", 0.0)
    net_profit = pnl.get("estimated_net_profit", 0.0)
    cit_tax = pnl.get("estimated_corporate_tax", 0.0)
    profit_color = "#16a34a" if net_profit >= 0 else "#dc2626"

    total_docs = doc_counts.get("total_documents", 0)
    drive_url = sanitize_flex_uri(drive_folders.get("root", "https://drive.google.com"))
    ...
    flex_bubble = {
        ...
        "header": {
            ...
            "paddingAll": "16px",
            "contents": [
                {"type": "text", "text": "CPA AUDIT & FINANCIAL CLOSING SUITE", "color": "#c7d2fe", "size": "xxs", "weight": "bold"},
                {"type": "text", "text": f"📑 ชุดเอกสารปิดงบการเงินประจำปี {target_year}", "color": "#ffffff", "size": "md", "weight": "bold", "margin": "xs"},
                {"type": "text", "text": "บจ. จีเอชเอ็น 168 มีเดีย แอนด์ ครีเอชั่น จำกัด", "color": "#e0e7ff", "size": "xxs", "margin": "xs"}
            ]
        },
        "body": {
            "type": "box",
            ...
            "contents": [
                # Box 1: P&L Snapshot
                {
                    ...
                    "borderWidth": "1px",
                    "contents": [
                        {"type": "text", "text": "📊 ภาพรวมรายได้ & กำไรสุทธิ (P&L Summary)", "size": "xs", "weight": "bold", "color": "#0f172a"},
                        {"type": "separator", "margin": "sm"},
                        {
                            "type": "box",
                            "layout": "horizontal",
                            "margin": "sm",
                            "contents": [
                                {"type": "text", "text": "• รายรับรวมทั้งปี (ก่อน VAT):", "size": "xxs", "color": "#64748b", "flex": 6},
                                {"type": "text", "text": f"{rev_pre:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#16a34a", "align": "end", "flex": 4}
                            ]
                        },
                        {
                            "type": "box",
                            ...
                            "margin": "xs",
                            "contents": [
                                {"type": "text", "text": "• รายจ่ายรวมทั้งปี (ก่อน VAT):", "size": "xxs", "color": "#64748b", "flex": 6},
                                {"type": "text", "text": f"{exp_pre:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#dc2626", "align": "end", "flex": 4}
                            ]
                        },
                        {
                            "type": "box",
                            ...
                            "margin": "xs",
                            "contents": [
                                {"type": "text", "text": "• กำไรสุทธิเบื้องต้น (Est. Net Profit):", "size": "xs", "weight": "bold", "color": "#0f172a", "flex": 6},
                                {"type": "text", "text": f"{net_profit:,.2f} ฿", "size": "xs", "weight": "bold", "color": profit_color, "align": "end", "flex": 4}
                            ]
                        },
                        {
                            "type": "box",
                            ...
                            "margin": "xs",
                            "contents": [
                                {"type": "text", "text": "• ประมาณการภาษีนิติบุคคล (ภ.ง.ด.50):", "size": "xxs", "color": "#64748b", "flex": 6},
                                {"type": "text", "text": f"{cit_tax:,.2f} ฿", "size": "xxs", "weight": "bold", "color": "#d97706", "align": "end", "flex": 4}
                            ]
                        }
                    ]
                },
                # Box 2: Document Inventory Breakdown
                {
                    "type": "box",
                    "layout": "vertical",
                    "margin": "sm",
                    ...
                    "contents": [
                        {
                            ...
                            "layout": "horizontal",
                            "contents": [
                                {"type": "text", "text": "🗂️ ทะเบียนคลังเอกสารส่ง CPA", "size": "xs", "weight": "bold", "color": "#1e40af", "flex": 7},
                                {"type": "text", "text": f"รวม {total_docs} ฉบับ", "size": "xxs", "weight": "bold", "color": "#2563eb", "align": "end", "flex": 3}
                            ]
                        },
                        {"type": "separator", "margin": "sm"},
                        {
                            ...
                            "margin": "sm",
                            "contents": [
                                {"type": "text", "text": "01_Quotation (ใบเสนอราคา):", "size": "xxs", "color": "#64748b", "flex": 7},
                                {"type": "text", "text": f"{doc_counts.get('quotations', 0)} ฉบับ", "size": "xxs", "color": "#1e293b", "align": "end", "flex": 3}
                            ]
                        },
                        {
                            "type": "box",
                            ...
                            "margin": "xs",
                            "contents": [
                                {"type": "text", "text": "02_Invoice (ใบวางบิล):", "size": "xxs", "color": "#64748b", "flex": 7},
                                {"type": "text", "text": f"{doc_counts.get('invoices', 0)} ฉบับ", "size": "xxs", "color": "#1e293b", "align": "end", "flex": 3}
                            ]
                        },
                        {
                            "type": "box",
                            ...
                            "margin": "xs",
                            "contents": [
                                {"type": "text", "text": "03_Receipt (ใบเสร็จ/ใบกำกับภาษี):", "size": "xxs", "color": "#64748b", "flex": 7},
                                {"type": "text", "text": f"{doc_counts.get('receipts_tax_invoices', 0)} ฉบับ", "size": "xxs", "color": "#1e293b", "align": "end", "flex": 3}
                            ]
                        },
                        {
                            "type": "box",
                            ...
                            "margin": "xs",
                            "contents": [
                                {"type": "text", "text": "04_WHT (หนังสือหัก ณ ที่จ่าย 50 ทวิ):", "size": "xxs", "color": "#64748b", "flex": 7},
                                {"type": "text", "text": f"{doc_counts.get('wht_certificates', 0)} ฉบับ", "size": "xxs", "color": "#1e293b", "align": "end", "flex": 3}
                            ]
                        },
                        {
                            "type": "box",
                            ...
                            "margin": "xs",
                            "contents": [
                                {"type": "text", "text": "05_Expenses (สลิป/ใบสำคัญจ่าย):", "size": "xxs", "color": "#64748b", "flex": 7},
                                {"type": "text", "text": f"{doc_counts.get('expenses_vouchers', 0)} ฉบับ", "size": "xxs", "color": "#1e293b", "align": "end", "flex": 3}
                            ]
                        }
                    ]
                },
                # Box 3: Audit Readiness
                {
                    "type": "box",
                    "layout": "horizontal",
                    "margin": "sm",
                    ...
                    "borderWidth": "1px",
                    "contents": [
                        {"type": "text", "text": f"🛡️ Audit Readiness Score: {readiness.get('score_percent', 100)}%", "size": "xs", "weight": "bold", "color": "#15803d", "flex": 8},
                        {"type": "text", "text": "พร้อมตรวจ 100%", "size": "xxs", "weight": "bold", "color": "#16a34a", "align": "end", "flex": 4}
                    ]
                }
            ]
        },
            ...
            "contents": [
                {
                    ...
                    "action": {
                        "type": "uri",
                        "label": "📂 เปิด Google Drive คลังปิดงบ CPA",
                        "uri": drive_url
                    }
                }
            ]
    ...
    return {
        "type": "flex",
        "altText": f"📑 ชุดเอกสารปิดงบ CPA GHN168 ประจำปี {target_year} (กำไรสุทธิ {net_profit:,.2f} ฿)",
        "contents": flex_bubble
    }
...
TAX_FILING_KEYWORDS = [
    "ยื่นภาษี", "ตัวเลขยื่นภาษี", "ขอยอดภาษี", "ภพ30", "ภ.พ.30", "ภงด3", "ภ.ง.ด.3",
    "ภงด53", "ภ.ง.ด.53", "e-filing", "efiling", "สรุปยื่นภาษี", "ยอดภาษีนำส่ง",
    "ยอดภาษีที่ต้องจ่าย", "รายงานภาษี", "ภาษีนำส่ง", "ยื่นภพ30", "ยื่นภงด", "สรุปภาษีเดือนนี้",
    "แบบยื่นภาษี", "ยื่นแบบ", "ภาษีสรรพากร", "สรรพากรเดือนนี้", "ตารางยื่นภาษี", "ภาษี rd"
]

CPA_AUDIT_KEYWORDS = [
    "ปิดงบ", "ปิดงบการเงิน", "ส่งผู้สอบบัญชี", "แพ็กเกจ cpa", "เอกสารปิดงบ", "audit package",
    "cpa package", "ปิดงบประจำปี", "ปิดงบปี", "สรุปเอกสารให้ cpa", "ผู้สอบบัญชี", "ส่งบัญชีปิดงบ",
    "สรุปข้อมูลปิดงบ", "คลังเอกสารปิดงบ", "cpa audit", "รายงานปิดงบ", "งบการเงินประจำปี", "แพ็กเกจปิดงบ"
]
...
def is_tax_filing_request(text: str) -> Tuple[bool, Optional[int], Optional[int], str]:
    """
    Detects if user message is an explicit tax filing (RD e-Filing) report request:
    Returns: (is_tax_filing, month, year, tax_type)
    tax_type can be 'all', 'pnd30', 'pnd3', 'pnd53'
    """
    if not text:
        return False, None, None, "all"

    text_lower = text.lower()
    if not any(kw in text_lower for kw in TAX_FILING_KEYWORDS):
        return False, None, None, "all"

    # Extract tax type
    tax_type = "all"
    if "30" in text_lower or "ภพ" in text_lower or "ภ.พ." in text_lower:
        tax_type = "pnd30"
    elif "53" in text_lower or "ภงด53" in text_lower or "ภ.ง.ด.53" in text_lower:
        tax_type = "pnd53"
    elif "3" in text_lower or "ภงด3" in text_lower or "ภ.ง.ด.3" in text_lower:
        tax_type = "pnd3"
    ...
    for m_k, m_v in month_map.items():
        if m_k in text_lower:
            month = m_v
            break

    # Year search (e.g. 2026 or 2569)
    y_match = re.search(r"\b(202\d|256\d)\b", text_lower)
    if y_match:
        y_val = int(y_match.group(1))
        year = y_val - 543 if y_val > 2500 else y_val

    return True, month, year, tax_type


def is_cpa_audit_request(text: str) -> Tuple[bool, Optional[int]]:
    """
    Detects if user message is an Annual CPA Audit Package / Financial Closing report request:
    Returns: (is_cpa_audit, year)
    """
    if not text:
        return False, None

    text_lower = text.lower()
    if not any(kw in text_lower for kw in CPA_AUDIT_KEYWORDS):
        return False, None
...
GEMINI_AGENT_TOOL_DECLARATIONS = [
    ...
    {
        "name": "get_tax_filing_report",
        "description": "Fetches monthly tax filing figures for Revenue Department (RD e-Filing) including ภ.พ.30 (VAT 7% sales/purchases tax breakdown & net payable), ภ.ง.ด.3 (individual withholding tax for freelancers/crews), and ภ.ง.ด.53 (corporate withholding tax for rental/services).",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "month": {"type": "INTEGER", "description": "Tax period month 1-12 (default current month)"},
                "year": {"type": "INTEGER", "description": "Tax period Christian Year e.g. 2026 (default current year)"},
                "tax_type": {"type": "STRING", "description": "Specific tax report: 'all' (default), 'pnd30' (ภ.พ.30 VAT 7%), 'pnd3' (ภ.ง.ด.3 บุคคล), 'pnd53' (ภ.ง.ด.53 นิติบุคคล)"}
            }
        }
    },
    {
        "name": "prepare_cpa_audit_package",
        "description": "Prepares complete annual CPA audit package and financial closing dataset (ปิดงบการเงินประจำปี) for Certified Public Accountant, including annual P&L revenue/expenses, estimated net profit, estimated CIT, categorized document counts, and Google Drive archive folders.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "year": {"type": "INTEGER", "description": "Christian Year e.g. 2026 (default current year)"}
...
def execute_agent_tool(
    ...
    elif func_name == "get_tax_filing_report":
        m = args.get("month") or None
        y = args.get("year") or None
        tax_t = args.get("tax_type") or "all"
        tax_res = get_tax_filing_report(month=m, year=y)
        flex_card = build_tax_filing_flex_message(tax_res, tax_type=tax_t)
        return {
            "status": "success",
            "tax_report": tax_res
        }, flex_card

    elif func_name == "prepare_cpa_audit_package":
        y = args.get("year") or None
        cpa_res = get_cpa_audit_package(year=y)
        flex_card = build_cpa_audit_pack_flex_message(cpa_res)
        return {
            "status": "success",
            "cpa_audit_package": cpa_res
        }, flex_card
...
async def agentic_fallback_simulate_turn(user_message: str, session_id: str, speaker_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 3. Tax Filing Report (RD e-Filing: ภ.พ.30, ภ.ง.ด.3, ภ.ง.ด.53)
    is_tax_f, t_m, t_y, t_type = is_tax_filing_request(clean_msg)
    if is_tax_f:
        res_data, flex_card = execute_agent_tool("get_tax_filing_report", {"month": t_m, "year": t_y, "tax_type": t_type}, session_id)
        tax_res = res_data.get("tax_report", {})
        pnd30 = tax_res.get("pnd30", {})
        pnd3 = tax_res.get("pnd3", {})
        pnd53 = tax_res.get("pnd53", {})
        summ = tax_res.get("summary", {})
        tot_to_pay = summ.get("total_tax_to_pay", 0.0)
        vat_pay = pnd30.get("vat_net_payable", 0.0)

        vat_status_txt = f"ต้องชำระ {vat_pay:,.2f} ฿" if vat_pay > 0 else (f"ภาษีซื้อยกไป {abs(vat_pay):,.2f} ฿" if vat_pay < 0 else "0.00 ฿")

        text_tax = (
            f"🏛️ สรุปตัวเลขยื่นภาษีประจำงวด {tax_res.get('period_label')} (RD e-Filing) ค่ะ{speaker_label} ✨\n\n"
            f"1. ภ.พ.30 (VAT 7%): ภาษีขาย +{pnd30.get('vat_output', 0.0):,.2f} ฿ | ภาษีซื้อ -{pnd30.get('vat_input', 0.0):,.2f} ฿ ➔ {vat_status_txt}\n"
            f"2. ภ.ง.ด.3 (บุคคลธรรมดา): ยอดจ่าย {pnd3.get('base_total', 0.0):,.2f} ฿ ➔ ภาษีหักนำส่ง {pnd3.get('wht_total', 0.0):,.2f} ฿ ({pnd3.get('count', 0)} ราย)\n"
            f"3. ภ.ง.ด.53 (นิติบุคคล): ยอดจ่าย {pnd53.get('base_total', 0.0):,.2f} ฿ ➔ ภาษีหักนำส่ง {pnd53.get('wht_total', 0.0):,.2f} ฿ ({pnd53.get('count', 0)} ราย)\n\n"
            f"💰 รวมภาษีที่ต้องนำส่งสรรพากรทั้งสิ้น: {tot_to_pay:,.2f} บาท\n"
            f"⏳ เดดไลน์ e-Filing: WHT 15/{tax_res.get('month', 1)+1 if tax_res.get('month', 1) < 12 else 1:02d} | VAT 23/{tax_res.get('month', 1)+1 if tax_res.get('month', 1) < 12 else 1:02d}"
        )
        append_to_history(session_id, "user", clean_msg)
        append_to_history(session_id, "model", text_tax)
        return {
            "reply_text": text_tax,
            "flex_cards": [flex_card] if flex_card else [],
            "tax_filing_result": tax_res,
            "executed_tools": [{"tool": "get_tax_filing_report", "args": {"month": t_m, "year": t_y, "tax_type": t_type}, "result": res_data}]
        }

    # 4. CPA Annual Audit Package (ชุดปิดงบการเงินประจำปี)
    is_cpa, c_year = is_cpa_audit_request(clean_msg)
    if is_cpa:
        res_data, flex_card = execute_agent_tool("prepare_cpa_audit_package", {"year": c_year}, session_id)
        cpa_res = res_data.get("cpa_audit_package", {})
        pnl = cpa_res.get("pnl_summary", {})
        d_counts = cpa_res.get("document_counts", {})
        readiness = cpa_res.get("audit_readiness", {})

        text_cpa = (
            f"📑 สรุปชุดเอกสารปิดงบการเงินประจำปี {cpa_res.get('year')} สำหรับส่งผู้สอบบัญชี (CPA Audit Package) ค่ะ{speaker_label} ✨\n\n"
            f"• รายรับรวมทั้งปี: {pnl.get('total_revenue_pre_vat', 0.0):,.2f} บาท\n"
            f"• รายจ่ายรวมทั้งปี: {pnl.get('total_expense_pre_vat', 0.0):,.2f} บาท\n"
            f"• กำไรสุทธิเบื้องต้น (Est. Net Profit): {pnl.get('estimated_net_profit', 0.0):,.2f} บาท\n"
            f"• ประมาณการภาษีนิติบุคคล (ภ.ง.ด.50): {pnl.get('estimated_corporate_tax', 0.0):,.2f} บาท\n"
            f"• คลังเอกสารทั้งหมด: {d_counts.get('total_documents', 0)} ฉบับ (QT: {d_counts.get('quotations', 0)}, IV: {d_counts.get('invoices', 0)}, RE: {d_counts.get('receipts_tax_invoices', 0)}, 50ทวิ: {d_counts.get('wht_certificates', 0)}, ค่าใช้จ่าย: {d_counts.get('expenses_vouchers', 0)})\n"
            f"• ความพร้อมเอกสาร (Readiness Score): {readiness.get('score_percent', 100)}% (พร้อมส่งมอบ CPA)"
        )
        append_to_history(session_id, "user", clean_msg)
        append_to_history(session_id, "model", text_cpa)
        return {
            "reply_text": text_cpa,
            "flex_cards": [flex_card] if flex_card else [],
            "cpa_audit_result": cpa_res,
            "executed_tools": [{"tool": "prepare_cpa_audit_package", "args": {"year": c_year}, "result": res_data}]
        }

    # 5. Accounting & Tax Summary (Live monthly summary / VAT / WHT)
