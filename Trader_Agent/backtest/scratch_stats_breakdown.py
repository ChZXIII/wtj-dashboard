...
print(f"Winning Trades: {len(winners)} ({len(winners)/len(df_trades)*100:.1f}%)")
print(f"Losing Trades: {len(losers)} ({len(losers)/len(df_trades)*100:.1f}%)")
print(f"Average Winner: +{winners['pnl'].mean():.2f}%")
print(f"Average Loser: {losers['pnl'].mean():.2f}%")
print(f"Win/Loss Ratio (Avg Win / Avg Loss): {winners['pnl'].mean() / abs(losers['pnl'].mean()):.2f}x")

# Compounding breakdown
print("\n=== TRADE DISTRIBUTION ===")
print(f"Huge Winners (> 50%): {len(df_trades[df_trades['pnl'] > 50])} trades")
print(f"Medium Winners (10% to 50%): {len(df_trades[(df_trades['pnl'] > 10) & (df_trades['pnl'] <= 50)])} trades")
print(f"Small Winners (0% to 10%): {len(df_trades[(df_trades['pnl'] > 0) & (df_trades['pnl'] <= 10)])} trades")
print(f"Small Losers (-5% to 0%): {len(df_trades[(df_trades['pnl'] > -5) & (df_trades['pnl'] <= 0)])} trades")
print(f"Medium/Big Losers (< -5%): {len(df_trades[df_trades['pnl'] <= -5])} trades")

print("\n=== TOP 5 LARGEST WINNING TRADES ===")
print(df_trades.sort_values("pnl", ascending=False).head(5).to_string())
