...
flat_long_sig = crossunder(ema_f, ema_s) & is_bull
flat_short_sig = crossover(ema_f, ema_s) # Any crossover exits SHORT

# Build positions with FLAT logic
...
for i in range(len(pos)):
    if long_sig.iloc[i]:
        ...
        current = -1
    elif flat_long_sig.iloc[i] and current == 1:
        current = 0
    elif flat_short_sig.iloc[i] and current == -1:
        current = 0
    pos[i] = current
